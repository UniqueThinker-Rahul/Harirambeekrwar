...
const Navbar = () => {
  ...
  return (
    <nav className="bg-white shadow-md sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16">
          ...
          <div className="hidden md:flex items-center space-x-6 lg:space-x-8">
            <Link to="/" className="text-medium-grey hover:text-secondary px-2 py-2 text-sm font-medium transition-colors">Home</Link>
            <Link to="/about" className="text-medium-grey hover:text-secondary px-2 py-2 text-sm font-medium transition-colors">About</Link>
            <Link to="/services" className="text-medium-grey hover:text-secondary px-2 py-2 text-sm font-medium transition-colors">Services</Link>
            
            {/* UPGRADED: Highlighted Urgent Love Link with pulsing dot indicator */}
            <Link to="/urgent-love-plan" className="group relative text-red-600 hover:text-red-700 px-4 py-1.5 text-sm font-bold transition-all bg-red-50 rounded-full border border-red-100 hover:border-red-200 flex items-center gap-2 shadow-sm hover:shadow">
            ...
            <Link to="/blog" className="text-medium-grey hover:text-secondary px-2 py-2 text-sm font-medium transition-colors">Blog</Link>
            <Link to="/contact" className="text-medium-grey hover:text-secondary px-2 py-2 text-sm font-medium transition-colors">Contact</Link>
      ...
      {isOpen && (
        <div className="md:hidden bg-white border-t border-gray-100 shadow-inner">
          <div className="px-4 pt-2 pb-4 space-y-1 sm:px-5">
            ...
            {/* UPGRADED: Highlighted Urgent Love Link for Mobile */}
            <Link to="/urgent-love-plan" onClick={() => setIsOpen(false)} className="text-red-600 bg-red-50 hover:bg-red-100 block px-3 py-2.5 rounded-lg text-base font-bold transition-colors flex items-center gap-2 border border-red-100 my-2">
      ...
      {/* UPGRADED: Blinking Call Banner wrapped in a Link for functionality & Scrolling Phone Number */}
      <div className="w-full flex flex-col">
        {/* Blinking Animation Element made clickable */}
